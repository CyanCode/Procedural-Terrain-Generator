...
namespace JBooth.MegaSplat.NodeEditorFramework 
{
	public static class RTEditorGUI 
	{
		...
		public static bool EndChangeCheck () 
		{
         ...
         return UnityEditor.EditorGUI.EndChangeCheck() || changed;
		}
      ...
      public static string TextArea(string text, float height)
      {
         ...
         UnityEditor.EditorStyles.textField.fontSize = 16;
         string t = UnityEditor.EditorGUILayout.TextArea(text, GUILayout.Height(height));
         UnityEditor.EditorStyles.textField.wordWrap = false;
         UnityEditor.EditorStyles.textField.fontSize = oldFontSize;
         return t;
      }
      ...
      public static Vector2 Vector2Field(string label, Vector2 val, params GUILayoutOption[] options)
      {
         return UnityEditor.EditorGUILayout.Vector2Field(label, val, options);
      }

      public static Vector3 Vector3Field(string label, Vector3 val, params GUILayoutOption[] options)
      {
         return UnityEditor.EditorGUILayout.Vector3Field(label, val, options);
      }
      ...
      #if UNITY_EDITOR
      public static AnimationCurve CurveField(AnimationCurve curve, params GUILayoutOption[] options)
      {
         return UnityEditor.EditorGUILayout.CurveField(curve, Color.green, new Rect(0, 0, 1, 1), options);
      }
      public static AnimationCurve CurveField(string label, AnimationCurve curve, params GUILayoutOption[] options)
      {
         return UnityEditor.EditorGUILayout.CurveField(label, curve, Color.green, new Rect(0, 0, 1, 1), options);
      }
      #endif
		...
		public static void DrawTexture (Texture texture, int texSize, params GUILayoutOption[] options) 
		{
			if (options == null || options.Length == 0)
				options = new GUILayoutOption[] { GUILayout.ExpandWidth (false) };
			Rect rect = GUILayoutUtility.GetRect (texSize, texSize, options);
		...
		private static void SetupLineMat (Texture tex, Color col) 
		{
			...
			if (tex == null)
            tex = lineTexture != null? lineTexture : lineTexture = JBooth.MegaSplat.NodeEditorFramework.ResourceManager.LoadTexture ("Resources/Textures/AALine.png");
