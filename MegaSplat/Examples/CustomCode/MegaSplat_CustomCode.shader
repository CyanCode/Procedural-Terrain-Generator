...
// insert Property definitions here. Properties must begin with _Custom_, so if you want a
// texture you could define one like

 _Custom_MyTexture("My Texture", 2D) = "black"
...
#include "megasplat_custom.cginc"
         ...
         float2 ProjectUVs(float3 vertex, float2 coords, float3 worldPos)
         {
            #if _UVLOCALTOP
         ...
         float2 ProjectUV2(float3 vertex, float2 coords, float3 worldPos)
         {
            #if _UVLOCALTOP2
         ...
         // water normal only
         half2 DoPuddleRefract(float2 uv, half puddleLevel, float2 flowDir, half height)
         {
            #if _PUDDLEFLOW || _PUDDLEREFRACT
            ...
            fd.xy = fd.xy * 2 - 1;
            half2 waterNorm = half2(fd.x, fd.y);
         ...
         float DoPuddles(inout MegaSplatLayer o, float2 uv, half puddleLevel, half2 flowDir, half porosity, float3 worldNormal)
         {
            #if _PUDDLES || _PUDDLEFLOW || _PUDDLEREFRACT
            puddleLevel *= _MaxPuddles;
            ...
            half4 fd = lerp(tex2D(_PuddleNormal, uv1), tex2D(_PuddleNormal, uv2), interp);
            fd.xy = fd.xy * 2 - 1;
            half3 waterNorm = half3(fd.x, fd.y, sqrt(1 - saturate(dot(fd.xy, fd.xy))));

            #if _PUDDLEFOAM
            half pmh = puddleLevel - o.Height;
            half foam = tex2D(_PuddleNormal, uv * 1.75 + fd.xy*fd.b + _Time.x * -flowDir).b;
            ...
            WaterBRDF(wetAlbedo, wetSmoothness, o.Metallic, waterBlend, porosity);
               ...
               float2 ruv2 = ruv1;
                  #if _PUDDLEFLOW || _PUDDLEREFRACT
                  ruv1 = uv1 * _RainUVScales.xy;
                  ruv2 = uv2 * _RainUVScales.xy;
                  #endif
         ...
         { 
            ...
            fixed4 albedo = tex0 * weights.x + tex1 * weights.y + tex2 * weights.z;
               ...
               #if _PUDDLEREFRACT
               float2 refractOffset = DoPuddleRefract(si.macroUV, si.puddleHeight, si.flowDir, albedo.a);
         ...
         MegaSplatLayer DoSurf(inout SplatInput si, MegaSplatLayer macro, float3x3 tangentToWorld)
         {
            ...
            float pud = 0;
            #if _PUDDLES || _PUDDLEFLOW || _PUDDLEREFRACT
               pud = DoPuddles(splats, si.macroUV, si.puddleHeight, si.flowDir, porosity, worldNormal);
            #endif



            #if _SNOW
               DoSnow(splats, si.macroUV, worldNormal, si.snowHeightFade, pud, porosity);
            #endif
      ...
      SplatInput ToSplatInput(Input i)
      {
         ...
         #endif
         #if _FLOW || _FLOWREFRACTION || _PUDDLEFLOW || _PUDDLEREFRACT
      ...
      void vert (inout appdata_full i, out Input o) 
      {
          ...
          /* Fix for parallax bug (well, unity's compiler bug), but uses too many interpolators
          #if _PARALLAX
          // DUE TO UNITY COMPILER BUG, we have to compute our own view dir
          // otherwise, it gets stripped
          ...
          o.viewDir = mul (rotation, WorldSpaceViewDir(i.vertex));
          #endif
          */

          o.coords.xy = ProjectUVs(i.vertex.xyz, o.coords.xy, worldPos);
          #if _SECONDUV
          o.macroUV.xy = ProjectUV2(i.vertex.xyz, o.macroUV.xy, worldPos);
          #endif
          ...
          #if _TRIPLANAR_WORLDSPACE || _SNOW || _PROJECTTEXTURE_WORLD || _PROJECTTEXTURE2_WORLD
