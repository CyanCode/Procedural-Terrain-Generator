...
namespace Terra.Terrain {
	...
	public class TerrainPaint {
		...
		public TerrainPaint(GameObject gameobject, List<SplatSetting> splatSettings) {
			...
			const string path = "Nature/Terrain/Standard-Flat";
		...
		/// </summary>
		/// <param name="isSmooth">Is this mesh a flat shaded mesh?</param>
		...
		public List<Texture2D> GenerateSplatmaps(bool isFlat, bool applySplats = true, bool debug = false) {
			...
			if (applySplats) {
				ApplySplatmapsToShaders(maps, isFlat);
			}
		...
		/// <param name="isFlat">Optionally make the applied textures flat-shaded</param>
		public void ApplySplatmapsToShaders(List<Texture2D> splats, bool isFlat) {
		...
		MeshSample SampleAt(float x, float z) {
			...
			float height = 0;
			float angle = 0;
			if (sampleLoc < Vertices.Length - 1) {
				height = Vertices[sampleLoc].y;
				angle = Vector3.Angle(Normals[sampleLoc], Vector3.up);
			}
