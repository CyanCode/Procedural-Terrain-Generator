...
namespace Terra.Graph.Noise.Modifier {
	...
	public class BiomeNode: XNode.Node {
		[Output]
		...
		/// <returns></returns>
		public float[] GetHeightsAt(float x, float y) {
		...
		public Texture2D Preview(int size) {
			...
			float[,,] heights = new float[size, size, 3];

			//Fill heights structure and set min/max values
			for (int x = 0; x < size; x++) {
				for (int y = 0; y < size; y++) {
					float[] generated = GetHeightsAt(x / (float)size, y / (float)size);
